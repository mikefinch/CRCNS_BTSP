...
peak_basal_plateau_prob_per_dt = 2.9 * peak_basal_plateau_prob_per_sec / 1000. * dt ############################################### 2.9 to get it closer to 0.5
...
print("Reward start times:", reward_start_times)
...
# CA3_input_rates = np.array(context.input_rate_maps)
...
for lap in range(10):
    ...
    print(multiInterp_ramp_population.shape)  # USING track x
    ...
    double_track_length_induction_gates, double_length_t = get_2d_induction_gate(plateau_start_times, track_t, int(plateau_dur / dt), lap_start_times[lap], dt)
    ...
    print(f"Lap: {lap}, prev_plateau_start_times before update: {prev_plateau_start_times}")
    prev_plateau_start_times = plateau_start_times
    print(f"Lap: {lap}, prev_plateau_start_times after update: {prev_plateau_start_times}")
