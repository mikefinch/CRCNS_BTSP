...
def get_plateau_probability2(multiInterp_ramp_population, population_representation_density2, prev_plateau_start_times, lap, plateau_dur, pause_dur, reward_dur, basal_plateau_prob_ramp_sensitivity, reward_plateau_prob_ramp_sensitivity, reward_start_times, reward_plateau_prob_f, basal_plateau_prob_f, lap_start_times, track_t, plateau_prob_ramp_sensitivity_f):
    ...
    for unit_id, this_plateau_start_times_list in enumerate(prev_plateau_start_times):
        for this_plateau_start_time in this_plateau_start_times_list:
            ...
            if len(plateau_indexes) > 0:
                print(f"unit_id {unit_id} plateau_stop_time {this_plateau_stop_time}")
...
def get_plateau_times2(track_t, dt, plateau_probability2, network_seed, lap, lap_start_times, plateau_dur, pause_dur,
                       debug=False):
    """
    ...
    plateau_start_times = []
    for i in range(success_matrix.shape[0]):  # iterating ober the cells (unit_ids)
        this_unit_plateau_start_times = []
        success_indices = np.where(success_matrix[i])[
            0]  # find the i'th row where there is a true - a cell that has a plateau time
        if debug:
            print(f"Cell {i} success_indices: {success_indices}")  # Debug
        if len(success_indices) > 1:
            if debug:
                print('getting here with row i:', i)
            intervals_between_successes = np.diff(
                success_indices)  # , prepend=success_indices[0]) #find the difference in unit id
        ...
        if debug:
            print(f"Cell {i} valid success_indices: {success_indices}")  # Debug
...
def update_weights2(two_track_length_ET, double_track_length_IS, current_weights, pot_rate, dep_rate, k_pot, k_dep, dt,
                    peak_delta_weight):
    start_time = time.time()
